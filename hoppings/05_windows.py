#!/usr/bin/env python3
"""
05 -- set the disentanglement windows from the actual Fermi level, and check
they are feasible before a run is spent on them.

Windows are given in the config as offsets from E_F, never as absolute
energies: each structure has its own E_F, and the same absolute window would
mean different physics in each.

Runs after the overlaps because the check needs .eig. Two conditions, both checked per k-point:
...
Usage:
    05_windows.py CONF                      # after nscf, before the main run
    05_windows.py CONF --ef 13.78           # override the detected E_F
    05_windows.py CONF --dry-run            # report only, do not touch .win
"""
import argparse
import glob
import os
...
def patch_win(path, vals, dry, stamp=""):
    ...
    text = re.sub(r"^!.*placeholder.*$",
                  f"! windows set by 05_windows.py, {stamp}",
