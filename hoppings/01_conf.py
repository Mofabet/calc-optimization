...
Usage:
    01_conf.py                          # scf.in / scf.out in the current dir
    01_conf.py -o system.conf
    01_conf.py -o /dev/null             # inspect only, write nothing
    01_conf.py --nscf-mesh 8 8 4 --projections 'Mn:d;Si:p'
"""
import argparse
import os
import re
...
def main():
    ...
    POLICY = ("PROJECTIONS", "DIS_WIN_LO", "DIS_WIN_HI", "DIS_FROZ_LO",
              ...
              "NBND_WINDOW", "NBND_MARGIN", "MESH_BASE", "W90_SRUN_OPTS", "OMP_THREADS", "DIS_CONV_TOL",
              "KPATH", "BANDS_NUM_POINTS",
    ...
    if eig_blocks and ef is not None:
        ...
        # The count cannot exceed the number of bands the SCF actually
        # computed. When it reaches that ceiling the SCF band set stops below
        # E_F+NBND_WINDOW and the count is only a lower bound.
        truncated = bool(nbnd) and hi >= nbnd - 2
        nbnd_nscf = hi + margin
    ...
    # Deliberately not capped at the SCF band count: the nscf diagonalises
    # afresh and may legitimately ask for more bands than the SCF carried.
    ...
    conf = f"""# generated by 01_conf.py from {args.input}
...
KPATH={dflt.get('KPATH', '')}
BANDS_NUM_POINTS={dflt.get('BANDS_NUM_POINTS', '100')}
...
OMP_THREADS={dflt.get('OMP_THREADS', '')}
"""
    if args.output == "-":
    ...
    if nbnd_nscf != nbnd:
        rel = (nbnd_nscf / nbnd) ** 2 if nbnd else 1.0
        how = (f".mmn about {1 - rel:.0%} smaller" if nbnd_nscf < nbnd
               else f".mmn about {rel:.1f}x larger")
        print(f"  nbnd       {nbnd} in the scf -> {nbnd_nscf} for the nscf"
              f"   ({nbnd_why}; {how})", file=e)
    if truncated:
        print(f"  WARNING: the scf carried only {nbnd} bands and they run out "
              f"below E_F+{win_ev:g} eV.", file=e)
        print(f"           NBND_NSCF={nbnd_nscf} is therefore a lower bound. "
              f"Raise nbnd in scf.in to at least", file=e)
        print(f"           {nbnd + 40}, or set NBND_NSCF by hand, or lower "
              f"NBND_WINDOW.", file=e)
    if dpath:
